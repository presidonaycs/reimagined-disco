...
const Page8TT = (props) => {
  ...
  console.log(description)
    ...
    const fetchDatas = async () => {
      let url = "get-admin-category" // 'Admin-Categoty';
      console.log('server')
       http.get(url,{params:{
...
const handleDone=()=>{
  http.post('create-admin-category', {
    id: 0,
    name: name ,
...
useEffect(()=>{
const func = tempData.forEach((item)=>{
  const globalData = {
    key: 0,
    name:'',
    description: '',
...
globalData.description = item.description;
globalData.key = item.id;
globalData.update = ``

tempArrays.push(globalData);
})
...
setData(tempArrays);
  ...
  const save = async (key) => {
    try {
      ...
      if (index > -1) {
        ...
        http.post('update-admin-category', {
          moduleName:'admin-inventory',
  ...
  const handleDelete = (key) => {
    ...
    http.delete("delete-admin-category", {
        params:{id:key}
    })
    .then(res=>console.log(res))
    // const index = newData.findIndex((item) => key === item.key);
  ...
  return (
   ...
   {/* <Option value="1">Not Identified</Option>
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                    ...
                                    >
                                        <Input style={{ width: "100%", border: "none", borderBottom: "1px solid", borderColor: "#D6D6D6" }} onChange={handleSequence}/>
                                    </Form.Item>
