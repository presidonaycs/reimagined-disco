...
function Page41(prop) {
    const { TextArea } = Input;
    const{show6, show22, handleShow6, handleShow22}= useState(ModalContext)
const handleChange= ()=>{
    handleShow22()
}

    return (
        <div className="page41 width flex" >
            <div style={{width:"100%" }}>
                <div className="padding" style={{ backgroundColor: "lightgreen", color: "black" }}>Request Approval</div>
                <div>
                    <TextArea bordered={false} rows={8}>
                ...
                <Form>
                    ...
                    >
                          <Button onClick={handleChange} block style={{ backgroundColor: "green", color: "white", marginRight:"10px", padding:"0 20px" }}>Submit Request</Button>
                    </Form.Item>
